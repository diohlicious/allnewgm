...
public class SuccessBiddingActivity extends GrosirMobilActivity {
    ...
    private void getHistoryTransaction(){
        ...
        HistoryTransactionRequest historyTransactionRequest = new HistoryTransactionRequest(1, 20, "1");
        ...
        historyTransactionApi.enqueue(new Callback<HistoryTransactionResponse>() {
                ...
                if (response.isSuccessful()) {
                    try {
                        if(response.body().getMessage().equals("success")){
                            RecyclerView.LayoutManager layoutManagerLive = new LinearLayoutManager(SuccessBiddingActivity.this);
                            rvBidSuccess.setLayoutManager(layoutManagerLive);
                            SuccessBiddingAdapter successBiddingAdapter = new SuccessBiddingAdapter(SuccessBiddingActivity.this, response.body().getDataPageHistoryTransactionResponse().getDataHistoryTransactionResponseList());
                            rvBidSuccess.setAdapter(successBiddingAdapter);
                            successBiddingAdapter.notifyDataSetChanged();
                        }else {
                            grosirMobilFunction.showMessage(SuccessBiddingActivity.this, "History Transaction", response.body().getMessage());
                        }
                    }catch (Exception e){
                    ...
                    try {
                        grosirMobilFunction.showMessage(SuccessBiddingActivity.this, getString(R.string.base_null_error_title), response.errorBody().string());
                    } catch (IOException e) {
                        GrosirMobilLog.printStackTrace(e);
                    }
                }
            ...
            public void onFailure(Call<HistoryTransactionResponse> call, Throwable t) {
                progress.setVisibility(View.GONE);
                grosirMobilFunction.showMessage(SuccessBiddingActivity.this, "History Transaction", getString(R.string.base_null_server));
